...
QUnit.test("Tween", (assert) => {
    const $target1 = getTarget(), startOpacity = $target1.style.opacity;
    assert.expect(11);
    assert.raises(() => Velocity("tween", "invalid"), "Invalid percentComplete throws an error.");
    assert.raises(() => Velocity([$target1, $target1], "tween", "invalid"), "Passing more than one target throws an error.");
    assert.raises(() => Velocity("tween", 0, ["invalid"]), "Invalid propertyMap throws an error.");
    assert.raises(() => Velocity("tween", 0, "invalid", 1), "Property without an element must be forcefed or throw an error.");
...
//# sourceMappingURL=Command Tween.js.map