...
QUnit.test("Speed", (assert) => {
    const delay = 200, duration = 400, startDelay = getNow();
    ...
    asyncTests(assert, 1, (done) => {
        Velocity(getTarget(), defaultProperties, {
            ...
            complete(elements) {
                const actual = getNow() - elements.__start, expected = duration / 3;
    ...
    asyncTests(assert, 1, (done) => {
        Velocity(getTarget(), defaultProperties, {
            ...
            complete(elements) {
                const actual = getNow() - elements.__start, expected = duration / 2;
    ...
    asyncTests(assert, 1, (done) => {
        Velocity(getTarget(), defaultProperties, {
            ...
            complete(elements) {
                const actual = getNow() - elements.__start, expected = (duration + delay) / 2;
